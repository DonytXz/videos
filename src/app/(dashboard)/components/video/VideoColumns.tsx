  ...
  return (
    <>
      <div className="flex flex-col py-4 px-2">
        <p className="tex-left mr-auto p-2 m-2 font-bold text-xl">Video coloumns</p>
        <div className="flex w-full m-2 p-2">
          <p className="text-left font-semibold mr-2">Time</p>
          <p className="text-left font-semibold">Description</p>
        </div>
        {videoDataArr.map((element: any, index: number) => {
          return (
            <div className={index == 1 ? "border-l-4 border-indigo-600 bg-gray-400 text-white" : ""} key={index}>
              <VideoColumn
                //   key={index}
                time={element.time}
